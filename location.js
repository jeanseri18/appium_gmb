    ...
    try {
        ...
        return data.map(item => {
            ...
            return {
                ...
                status: item.history.status === 1,  // Convertir le statut 1 à true
            };
        });
    } catch (error) {
...
async function searchOnGoogle(driver, keyword, agence_name, url, phone) {
    try {
        ...
        if (destinationDetails) {
            ...
            if (await inputField.isExisting()) {
                ...
                const startLocation = { latitude: startLatitude, longitude: startLongitude }; // Paris, France
                const endLocation = { latitude: endLatitude, longitude: endLongitude }; // Proche de la Tour Eiffel

                // Générer les étapes intermédiaires en ligne droite
                const steps = getIntermediatePoints(startLocation, endLocation, numPoints); // 8 points intermédiaires
                ...
                const buttonSelector = 'button[aria-label="Utiliser la navigation dans l\'application"]';
                //  if (await buttonSelector.isExisting()) {
                // Cliquer sur le bouton
                const button = await driver.$(buttonSelector);
                await button.click();

                simulateMovement(directions,endLatitude,endLocation,driver);
...
async function simulateMovement(directions,endLatitude,endLocation,driver) {
    ...
    const intervalId = setInterval(() => {
        if (index >= steps.length) {
            ...
            const command = `adb shell am start-foreground-service --user 0 -n io.appium.settings/.LocationService --es longitude ${endLocation} --es latitude ${endLatitude}`;
            exec(command, (error, stdout, stderr) => {
                if (error) {
                    console.error(`Erreur lors de l'exécution de la commande ADB: ${error}`);
                    return;
...
async function runTestOnDevice(device) {
    const { deviceName, udid, keyword, agence_name, url, phone, status } = device;
    ...
    try {
        ...
        if (false) {
            ...
            await searchOnGoogle(driver, keyword, agence_name, url, phone);
        }
    } catch (error) {
